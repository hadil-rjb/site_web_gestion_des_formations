                    ...
                    <div class="shadow-xs rounded-lg overflow-hidden w-full">
                      <div class="overflow-x-auto w-full p-4">
                        <table id="example" class="w-full border-collapse bg-white text-left text-sm text-gray-500">
                            <thead>
                                <tr>
                                    <th scope="col" class="px-6 py-4 font-medium text-gray-900">Nom cabinet</th>
                                    <th scope="col" class="px-6 py-4 font-medium text-gray-900">Email</th>
                                    <th scope="col" class="px-6 py-4 font-medium text-gray-900">Adresse</th>
                                    <th scope="col" class="px-6 py-4 font-medium text-gray-900">Tél</th>
                                    <th scope="col" class="px-6 py-4 font-medium text-gray-900">Web site</th>
                            ...
                            <tbody class="divide-y divide-gray-100 border-t border-gray-100">
                                @foreach ($cabinets as $cabinet)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-6 py-4">{{ $cabinet->name }}</td>
                                        <td class="px-6 py-4">{{ $cabinet->email }}</td>
                                        <td class="px-6 py-4">{{ $cabinet->address }}</td>
                                        <td class="px-6 py-4">{{ $cabinet->phone_number }}</td>
                                        <td class="px-6 py-4">{{ $cabinet->website }}</td>


                                        <td class="px-6 py-4">
                                            <div class="flex justify-end gap-4">
                                                <a href="{{ route('cabinets.edit', $cabinet) }}" class="px-2 py-1"><svg
                                                ...
                                                <form action="{{ url('/cabinet' . '/' . $cabinet->id) }}" method="POST"
